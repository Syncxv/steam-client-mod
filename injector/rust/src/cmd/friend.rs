...
pub async fn handle_friend_command_unpatch(steam_config: HashMap<String, String>) {
    unpatch_friends(&steam_config).await;
}

pub async fn handle_friend_command_patch(steam_config: HashMap<String, String>, update: bool) {
    patch_friend_javascript(&steam_config, update)
        .await
        .expect("Failed to patch friends");
}

pub async fn patch_friend_javascript(
    ...
    if update || should_update_friends_web_ui(&friends_web_ui).await {
        println!("[Friends Injector] Updating friends_web_ui.js");
        update_friends_webuijs(&friends_web_ui).await;
    } else {
    ...
    let steamed = fs::read_to_string(&steamed_path).await?;
    fs::write(clientui.join("steamed.js"), steamed)
        .await
        .unwrap();
    ...
    let steamed_js_injector_path = Path::new(&curr_dir)
        ...
        .join("js-injector")
        .join("injector.js");
    let steamed_js_injector = fs::read_to_string(&steamed_js_injector_path).await.unwrap();
    fs::write(&clientui.join("bruh.js"), steamed_js_injector)
        .await
        .expect("Failed to write js-injector");

    println!("[Friends Injector] inserted js-injector to clientui folder");

    //insert srcipt tag into html
    let mut index_html = fs::read_to_string(&steam_friend_index_html)
        .await
        .expect("FAILED TO READ FRIEND INDEX HTML");
    ...
    fs::write(&steam_friend_index_html, index_html)
        .await
        .expect("FAILED TO WRITE TO FRIEND INDEX HTML");

    println!("[Friends Injector] injected js-injector into html");

    //patching friends.js :)
    let patched = get_patch().await;

    let mut steam_friend_js = fs::read_to_string(&clientui.join("friends.js"))
        .await
...
async fn unpatch_friends(config: &HashMap<String, String>) {
    let steam_path_str = config.get("steam_path").unwrap();
    let steam_path = Path::new(steam_path_str);
    let clientui = steam_path.join("clientui");
    let steam_friend_index_html = clientui.join("index_friends.html");
    ...
    let patched = get_patch().await;
    let steam_friend_js = fs::read_to_string(&clientui.join("friends.js"))
        .await
        .expect("FAILED TO GET STEAM_FRIEND_JS");
    ...
    let index_html = fs::read_to_string(&steam_friend_index_html)
        .await
        .expect("FAILED TO READ FRIEND INDEX HTML");
    ...
    fs::write(&steam_friend_index_html, original)
        .await
        .expect("FAILED TO REVERT HTML");
...
async fn should_update_friends_web_ui(path: &Path) -> bool {
    if !path.exists() {
        return true;
    }

    let last_modified = fs::metadata(path).await.unwrap().modified().unwrap();
...
async fn update_friends_webuijs(friends_web_ui: &Path) {
    ...
    if let Ok(response) = response {
        if let Ok(content) = response.text().await {
            if let Err(e) = fs::write(friends_web_ui, content).await {
                eprintln!(
