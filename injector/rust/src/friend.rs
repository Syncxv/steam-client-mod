...
pub fn inject_friend_javascript(config: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    .text()?;
    fs::write(
        Config::join(&config.steam_client_ui, &["friends_web_ui.js"]),
        bruh,
    )
    .unwrap();
    ...
    fs::write(
        Config::join(&config.steam_client_ui, &["steamed.js"]),
        steamed,
    )
    .unwrap();
    ...
    let steamed_js_injector_path =
        (&config.steamed).to_string() + "\\injector\\js-injector\\injector.js";
    let steamed_js_injector = fs::read_to_string(&steamed_js_injector_path).unwrap();
    fs::write(
        Config::join(&config.steam_client_ui, &["bruh.js"]),
        steamed_js_injector,
    )
    .unwrap();
    ...
    let mut index_html = fs::read_to_string(&config.steam_friend_index_html)
    ...
    fs::write(&config.steam_friend_index_html, index_html)
    ...
    let mut steam_friend_js =
        fs::read_to_string(&config.steam_friend_js).expect("FAILED TO GET STEAM_FRIEND_JS");
    let index: usize = steam_friend_js
        .find(r#"console.log('Loading chat from url: ', strURL);"#)
        .unwrap();

    let hehe = fs::read_to_string(Config::join(
        &config.steamed,
        &["dist", "js", "iframe-patcher.js"],
    ))
...
pub fn is_friend_backed_up(
    steam_friend_js_bak: &String,
    steam_friend_index_html_js_bak: &String,
) -> bool {
...
pub fn backup_friend_assets(
    steam_friend_js: &String,
    steam_friend_index_html: &String,
    steam_friend_js_bak: &String,
    steam_friend_index_html_bak: &String,
) -> bool {
    if !is_friend_backed_up(&steam_friend_js_bak, &steam_friend_index_html_bak) {
        fs::copy(&steam_friend_js, &steam_friend_js_bak)
            .expect("[Friends Injector] failed backing up friends.js");
        fs::copy(&steam_friend_index_html, &steam_friend_index_html_bak)
            .expect("[Friends Injector] failed backing up index_friends.html");
        return false;
    }
...
pub fn delete_backups(steam_friend_js_bak: &String, steam_friend_index_html_bak: &String) -> bool {
    if is_friend_backed_up(&steam_friend_js_bak, &steam_friend_index_html_bak) {
        fs::remove_file(&steam_friend_js_bak)
            .expect("[Friends Injector] failed removing backup friend.js.bak");
        fs::remove_file(&steam_friend_index_html_bak)
            .expect("[Friends Injector] failed removing backup up index_friends.html.bak");
        return false;
    }
